...
<c:set var="campingList" value="${campingMap.campingList}" />
<c:set var="total" value="${campingMap.total}" />
<c:set var="section" value="${campingMap.section}" />
<c:set var="pageNum" value="${campingMap.pageNum}" />
<!DOCTYPE html>
<html lang="kr">
<head>
			...
			<div class="row">
				<div class="col-12">
					...
					<c:choose>
						...
						<c:when test="${campingList!=null}">
							<c:forEach var="list" items="${campingList}">		
									<tr>
										<td class="d-none d-lg-table-cell">${list.boardNo}</td>
										<td class="text-left">
											<c:if test="${list.boardRe_lev > 0}">
												<img src="${contextPath}/images/re.gif" style="margin-left:${list.boardRe_lev*10}px" class="mr-2" />
											</c:if>
											${list.boardTitle}
											<c:if test="${list.boardFileName!=null}">
										...
										<td class="d-none d-lg-table-cell"><fmt:formatDate value="${list.boardWriteDate}" pattern="yy-MM-dd"/></td>
										<td class="d-none d-lg-table-cell">${list.boardReadCount}</td>
									</tr>
							</c:forEach>
						</c:when>
					</c:choose>
					...
					<%-- 	<div class="row  my-5">
						...
						<div class="row">
							...
							<c:if test="${total != null}">
								<c:if test="${section > 1}">						
								   	<li class="page-item">
										<a class="page-link" href="${contextPath}/camp/listCamping.do?section=${section-1}&pageNum=1">이전</a>
									</li>
								</c:if>									
						<c:forEach var="page" begin="1" end="10" step="1">
							<c:if test="${page <= Math.ceil((total - (section-1)*100) /10)}">
								<c:choose>						
									<c:when test="${page==pageNum}">
										<li class="page-item active">
											<a class="page-link" href="${contextPath}/camp/listCamping.do?section=${section}&pageNum=${page}">${(section-1)*10 + page}</a>							
										</li>
									</c:when>
									<c:otherwise>
										<li class="page-item">
								...
								<c:if test="${Math.ceil(total/100) > section}">						
								   	<li class="page-item">
										<a class="page-link" href="${contextPath}/camp/listCamping.do?section=${section+1}&pageNum=1">다음</a>
									</li>
								</c:if>
							</c:if>
								</ul>
